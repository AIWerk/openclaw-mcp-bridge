...
$ServerConfigFile = Join-Path $ScriptDir "config.json"
$EnvVarsFile = Join-Path $ScriptDir "env_vars"
    ...
    switch ($ServerName) {
        ...
        "wise" {
            ...
            if (Test-Path (Join-Path $cloneDir ".git")) {
                Write-Host "Updating wise mcp-server source..."
                git -C $cloneDir pull --ff-only | Out-Host
            } else {
                Write-Host "Cloning wise mcp-server source..."
        ...
        "hetzner" {
            ...
            New-Item -ItemType Directory -Force -Path $parent | Out-Null
            if (Test-Path (Join-Path $cloneDir ".git")) {
                Write-Host "Updating hetzner mcp-server source..."
                git -C $cloneDir pull --ff-only | Out-Host
            } else {
                Write-Host "Cloning hetzner mcp-server source..."
...
function Get-PathOverride {
    switch ($ServerName) {
        ...
        "wise" {
            return (Join-Path $env:USERPROFILE ".openclaw\extensions\mcp-client\servers\wise\mcp-server\dist\cli.js")
        }
        "hetzner" {
            return (Join-Path $env:USERPROFILE ".openclaw\extensions\mcp-client\servers\hetzner\mcp-server\dist\index.js")
        }
        default {
            return ""
        }
...
if (-not (Test-Path $EnvVarsFile)) {
    throw "Missing env_vars file in $ScriptDir"
}
...
if (-not (Test-Path $OpenclawJson)) {
    Set-Content -Path $OpenclawJson -Value "{}" -Encoding UTF8
}
...
if ($allow -notcontains "mcp-client") {
    $plugins.allow = @($allow) + "mcp-client"
}
